...
適切なツールを呼び出す能力を備えています。SoulTavern をインストールしさえ
...
それで全部です。Hermes は意図を解釈し、裏で `soultavern` を呼び出し、
...
`SOULTAVERN_BIN` でパスを上書き可能。`soultavern` が PyPI に公開
...
hermes skills uninstall soultavern               # skill を削除
```
...
- Hermes プロセスの起動・監視
- `MEMORY.md` / `USER.md` / `CLAUDE.md` への書き込み(`AGENTS.md` も
  `--target hermes` では書きません;`--target openclaw` では managed-section
  形式で追記し、マーカー外のユーザー記述は保持します)
...
(これは `--target hermes` の予算です; OpenClaw の予算は異なります ——
`references/openclaw-target.md` を参照)。
...
SoulTavern は **独立した LLM を shell out しません。** `import` は
...
- `AGENTS.md` —— Hermes のローダー優先度により HERMES.md に隠される
  (`--target openclaw` では `AGENTS.md` に managed-section を追記し、
  マーカー外のユーザー記述は保持します)
...
```
SoulTavern/
├── src/soultavern/                Python パッケージ(エンジン本体; PyPI 公開までは同梱 wheel)
...
> リブランドし、複数 target サポート(`--target hermes` と
> `--target openclaw`)を導入しました。Python パッケージ名は
> `soultavern`、CLI バイナリ名も `soultavern`(`hermes-tavern` は
> 後方互換エイリアスとして残ります)。`hermes skills list` に
> まだ `hermes-tavern` が表示されている場合は、
> `hermes skills uninstall hermes-tavern` を実行してから上記の
> 手順で再インストールしてください。

> **v0.5.0 注記。** さらに以前のバージョンではライブラリ管理を別 skill
> `hermes-tavern-cards` として配布していました。v0.5.0 で本 skill
> (現在は `soultavern`)に統合済みです。もし `hermes skills list` に
> まだ `hermes-tavern-cards` が残っていれば、
> `hermes skills uninstall hermes-tavern-cards` で外してください。
...
- **チャンネルレベルの安全制御はありません。** Hermes 側で設定してください
  (`platform_toolsets`、許可リスト、レート制限)。SoulTavern は
  ペルソナファイルを書くだけです。
...
- **ポリシー制限の強い agent では、大きなカードのカテゴリ分けが部分的に
  終わることがあります。** カードが 15k 閾値を超えると、SoulTavern は
