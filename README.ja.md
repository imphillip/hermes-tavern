...
pip install pytest ruff mypy   # 唯一の依存; ランタイムは stdlib のみ
...
`from soultavern.parse import load_card` のような import は直接動きます。
v2.0 はインストール可能なパッケージとしては配布されません ——
`pyproject.toml` は `ruff` / `mypy` / `pytest` の設定ファイルとしてのみ残しています。
