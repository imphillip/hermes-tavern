...
- **大きなカードの agent 駆動フロー** —— カードのレンダリング結果が
  ...
  `hermes-tavern finalize` が精選 SOUL.md とインデックス化された
  HERMES.md を組み立てます。
...
書き終えたら、`hermes-tavern finalize` が最終的な SOUL.md(少数の
...
- [`oversized-cards.md`](skills/hermes-tavern/references/oversized-cards.md) — 大きなカードの agent 駆動分配フロー
- [`library-layout.md`](skills/hermes-tavern/references/library-layout.md) — `<HERMES_HOME>/cards/` schema、スナップショット、`--card` 解決
...
- **ポリシー制限の強い agent では、大きなカードのカテゴリ分けが部分的に
  ...
  `source.md` を再処理してから `hermes-tavern finalize` を再実行
  してください。
